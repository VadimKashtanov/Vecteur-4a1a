...
static __global__ void d_kerd_batch_miu(
	...
	uint C0)
{
	...
	if (_x < X0 && _t < GRAND_T) {
		...
		//
		uint LEN_L = X0 / C0;
		atomicAdd(&dx0[tx0*X0 + _x], dy[0*Y + c0] / (float)(LEN_L*GRAND_T));
	};
}
...
void batch_miu__df(Inst_t * inst, float ** x__d, float ** dx__d, uint * ts__d, uint mega_t) {
	...
	//
	if (x0_existe) {
		d_kerd_batch_miu<<<dim3(KERD(inst->x_Y[0],16), KERD(GRAND_T,16)), dim3(16,16)>>>(
