...
static __global__ void kerd__matmul_poid_PA__simple(
...
void matmul_poid_PA__f(Inst_t * inst, float ** x__d, uint * ts__d, uint mega_t, uint entrainnement) {
	...
	//
	if (x0_existe) {
		kerd__matmul_poid_PA__simple<<<dim3(KERD((Ay*C0),16), KERD((Bx*GRAND_T),16)), dim3(16,16)>>>(
