...
static __global__ void d_kerd__QKtDivClef__simple(
	...
	uint Ax, uint Ay, uint Bx, uint C0)
{
	...
	if (_ay < Ay && _c0 < C0 && _t < GRAND_T) {
		...
		float _dy = dy[pos_y] / sqrtf(1*(float)Ax);
		//
		FOR(0, k, Ax) {
			...
			atomicAdd(&dx0[pos_x0],           x1[pos_x1_transpose] * _dy);
			atomicAdd(&dx1[pos_x1_transpose], x0[pos_x0]           * _dy);
		}
	}
};
...
void QKtDivClef__df(Inst_t * inst, float ** x__d, float ** dx__d, uint * ts__d, uint mega_t) {
	...
	if (x0_existe && x1_existe) {
		d_kerd__QKtDivClef__simple<<<dim3(KERD((Ay*C0),16), KERD((Bx*GRAND_T),16)), dim3(16,16)>>>(
