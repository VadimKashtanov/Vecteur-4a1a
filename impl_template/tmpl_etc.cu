...
static char * copy_str(char * str, uint taille) {
	char * mot = (char*)malloc(taille + 1);
	memcpy(mot, str, taille);
	mot[taille-1 +1] = '\0';
	return mot;
};

static char ** split(char * text, char par) {	//",ds,aeaz,azddca,az," -> ["", "ds","aeaz","azddca","az",""]
	uint elements = 0;
	uint _strlen = 0;//strlen(text);
	uint _p=0;
	while (text[_p] != '\0') {
		if (text[_p] == par) elements++;
		_strlen++;
		_p++;
	}
	//
	uint pos[elements];
	uint pos_pos = 0;
	FOR(0, i, _strlen) if (text[i] == par) pos[pos_pos++] = i;
	//
	uint chars = elements+1;
	char ** ret = (char**)malloc(sizeof(char*) * chars);
	ret[       0       ]     = copy_str(text                   , pos[0]);
	FOR(0, i, elements-1) {
		ret[1+     i       ] = copy_str(text +pos[i]+1         , pos[i+1]-pos[i]-1);
	}
	ret[1+elements -1]       = copy_str(text +pos[elements-1]+1, _strlen-pos[elements-1]);
	//
	return ret;
};
